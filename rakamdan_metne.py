...
class RakamdanMetne(object):
	...
	def cevir(self, sayi):
		...
		self.sayi = str(sayi) # self.sayi -> string
		self.b_sayisi = self._kacBasamak(self.sayi) # self.b_sayisi -> int

		print("[+] Girilen sayı {} basamaklıdır.".format(self.b_sayisi))

		if self.b_sayisi > 3:
			self.f_sayi = "{:_}".format(sayi) # self.f_sayi -> string
			self.l_sayi = self.f_sayi.split("_") # self.l_sayi -> list
		else:
			self.l_sayi = []
